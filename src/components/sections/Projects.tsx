...
const Projects = () => {
  ...
  return (
    <section
      id="projects"
      className={`py-16 transition-all duration-300 ${isDarkMode ? "bg-gray-900 text-white" : "bg-gray-50 text-gray-900"
        }`}
    >
      <motion.h2
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
        {projects.map((project, index) => (
          <motion.div
            key={index}
            className={`relative group overflow-hidden rounded-lg shadow-lg cursor-pointer ${isDarkMode ? "bg-gray-800" : "bg-white"
              }`}
      ...
      {selectedProject && (
        ...
        >
          <motion.div
            className={`p-6 rounded-lg shadow-xl max-w-md w-full text-center transition-all duration-300 ${isDarkMode ? "bg-gray-800 text-white" : "bg-white text-gray-900"
              }`}
            initial={{ scale: 0.8 }}
            animate={{ scale: 1 }}
            transition={{ duration: 0.3 }}
